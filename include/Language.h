...
public:
    ...
    /*
     @brief returns the variable MAGIC_STRING_T
     @return MAGIC_STRING_T*/
    const std::string& getMagicStringT() const;

    /*
     @brief returns the variable MAGIC_STRING_B
     @return MAGIC_STRING_B*/
    const std::string& getMagicStringB() const;
    ...
    /**
     * @brief Saves this Language binary object in the given file
     * @param fileName A c-string with the name of the file where this Language 
     * object will be saved
     * @throw std::ios_base::failure Throws a std::ios_base::failure exception 
     * if the given file cannot be opened or if an error occurs while writing
     ...
     * @return A reference to the BigramFreq object at position @p index
     */
    const BigramFreq& operator[](const int &index) const;
...
private:
    ...
    /**
     @brief Frees up used dynamic memory
     */
    void liberarM();

    /**
     * @brief Reserves the @param tam amount of memory entered
     * and assigns it to _size
     */
    void asignarM(const int &tam);
};
